...
Date: 2026-06-26
Scope: generic work unit schema, WorkBoard UI, workflow linkage, and PM-tool adapter boundary
...
work that can be performed by humans, agents, or both. It should be useful on
its own through a built-in WorkBoard, while staying easy to map to external PM
tools such as Jira, Trello, ClickUp, Plane, Lark, GitHub Issues, or enterprise
...
The Work Module and WorkBoard are optional layers over DeerFlow's runtime:
...
- `work.enabled=false` disables Work Unit store construction and Work Unit API
  exposure.
- `work_board.enabled=false` hides the built-in WorkBoard UI while leaving
  lower-level runtime/work APIs available when their own gates are enabled.
- WorkBoard depends on Work Module, but Work Module does not depend on
  WorkBoard.
...
### Work PR 1: Architecture And Schema
...
- Create a local work unit.
- Move status through the API.
