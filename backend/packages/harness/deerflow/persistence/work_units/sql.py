...
class WorkUnitRepository(WorkUnitStore):
    ...
    async def update(self, work_unit_id: str, **kwargs: Any) -> dict[str, Any] | None:
        async with self._sf() as session:
            row = await session.get(WorkUnitRow, work_unit_id)
            if row is None:
                return None
            ...
            for key, value in kwargs.items():
                ...
                if key == "metadata":
                    ...
                    values[key] = value
            await session.execute(update(WorkUnitRow).where(WorkUnitRow.work_unit_id == work_unit_id).values(**values))
            await session.commit()
